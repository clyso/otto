...
@add_check
def check_report_header(result, data):
...
def check_report_version(result, data):
    ...
    return _handle_non_recommended_version(result, ver, recommended_versions(), release_info.get('version'))

def _handle_very_old_version(result, ver, rec_versions):
    summary = f"CRITICAL: Running very old release {ver}"
    detail = [f"Cluster is running a very old release {ver}. Clyso highly recommends upgrading to one of the stable releases: {', '.join(rec_versions)}."]
...
def _handle_recommended_version(result, ver):
    summary = f"Running a recommended stable release {ver}"
    detail = [f"Cluster is running {ver}. This is one of the recommended stable releases."]
    result.add_check_result('Version', 'Release', 'PASS', summary, detail, [])

def _handle_non_recommended_version(result, ver, rec_versions, rec_minor):
    summary = f"Not running a recommended stable release"
    compatible_versions = [v for v in rec_versions if version.parse(v) >= version.parse(ver)]
    
    if compatible_versions:
        detail = [f"Cluster is running {ver}. Clyso highly recommends one of the stable releases: {', '.join(compatible_versions)}."]
        recommend = [f"Upgrade to one of the stable releases: {', '.join(compatible_versions)}."]
    else:
        detail = [f"Cluster is running {ver}, which is newer than our recommended versions. Please ensure you're running a stable release."]
        recommend = ["Your version is newer than our recommended versions. It is not possible to downgrade to a previous release so we recommend waiting for a stable release."]

    if rec_minor and version.parse(ver) < version.parse(rec_minor):
        detail.append(f"{rec_minor} is the recommended bugfix release for your current version.")
...
@add_check
def check_report_known_bugs(result, data):
    report = data.ceph_report
    section = 'Version'
    ...
    summary = f"No known severe bugs in running release"
    ...
    for bug in bugs:
        ...
        detail.append(f"{bug['name']} (severity: {bug['severity']}): {bug['description']}")
        recommend.append(f"{bug['name']} (severity: {bug['severity']}): {bug['recommendation']}")
...
@add_check
def check_report_mixed_versions(result, data):
    report = data.ceph_report
    section = 'Version'
    ...
    cluster_version = report['version']
    ...
    if len(versions) == 0:
        ...
        detail.append(f"It was not possible to determine the running Ceph versions.")
        recommend.append(f"Contact support@clyso.com for assistance.")
    elif len(versions) == 1:
        passfail = 'PASS'
        ...
        detail.append(f"Found several versions of Ceph in the cluster, namely {','.join(versions)}. This may lead to unexpected issues, including data corruption in the worst case.")
        recommend.append(f"Upgrade all Ceph daemons to run the same version.")

    result.add_check_result(section, check, passfail, summary, detail, recommend)


@add_check
def check_report_health(result, data):
    report = data.ceph_report
    section = 'Cluster'
    check = 'Health'
    recommend = []
    ...
    for c, d in health['checks'].items():
        detail.append(f"Internal health check {c} with severity {d['severity']} reports {d['summary']['message']}")
        advice = healthdb['warnings'].get(c)
        if advice:
            recommend.append(advice)

    if health['status'] == 'HEALTH_WARN': passfail = 'WARN'
    if health['status'] == 'HEALTH_ERR': passfail = 'FAIL'
    result.add_check_result(section, check, passfail, summary, detail, recommend)
...
@add_check
def check_monmap_epoch(result, data):
    report = data.ceph_report
    section = 'MON Health'
    ...
    if monmap_epoch > 100:
        ...
        detail = [f'This cluster has a very large number of monmaps ({monmap_epoch}). If this number is increasing rapidly, it may indicate a problem. Please contact support.']
        passfail = 'WARN'
    else:
        summary = 'Correct number of monmaps'
        detail = [f'This cluster has a reasonably small number of monmaps ({monmap_epoch}), which is normal.']
        passfail = 'PASS'
    result.add_check_result(section, check, passfail, summary, detail, recommend)
    return
...
@add_check
def check_num_mons(result, data):
    report = data.ceph_report
    section = 'MON Health'
    ...
    if num_mons > 5:
        summary = f"Too many Monitors"
        detail = [f"Cluster has too many ceph-mon daemons ({num_mons}). Running more than 5 ceph-mons is not recommended in most environments due to the overheads keeping MONs in sync."]
        recommend = ["Deprovision ceph-mon daemons until you have five or fewer."]
        passfail = 'WARN'
        result.add_check_result(section, check, passfail, summary, detail, recommend)
    elif num_osds > 1000 and num_mons < 5:
        summary = f"Insufficient Monitors for large cluster"
        detail = [f"Cluster has insufficient ceph-mon daemons ({num_mons}) for the large number of OSDs ({num_osds}). Above 1000 OSDs, it is recommended to use 5 monitors."]
        recommend = ["Spawn additional ceph-mon daemons until you have five."]
        passfail = 'WARN'
        result.add_check_result(section, check, passfail, summary, detail, recommend)
    elif num_mons < 3:
        summary = f"Insufficient Monitors"
        detail = [f"Cluster has insufficient ceph-mon daemons ({num_mons}). Running fewer than 3 monitors means that each ceph-mon is a single point of failure."]
        recommend = ["Spawn additional ceph-mon daemons until you have at least three."]
        passfail = 'FAIL'
        result.add_check_result(section, check, passfail, summary, detail, recommend)
    else:
        summary = f"Sufficient Number of Monitors"
        detail = [f"Cluster has a sufficient number of ceph-mon daemons ({num_mons}) for its size."]
        recommend = []
        passfail = 'PASS'
        result.add_check_result(section, check, passfail, summary, detail, recommend)

@add_check
def check_even_num_mons(result, data):
    report = data.ceph_report
    section = 'MON Health'
    ...
    num_mons = len(report['monmap']['mons'])
    if num_mons % 2 == 0:
        summary = f"Even number of Monitors"
        detail = [f"Cluster has an even number of ceph-mon daemons ({num_mons}). This is not a problem however operators should be aware that the {ceil(num_mons/2+1)} of the monitors must be up to maintain quorum."]
        recommend = [f"Run an odd number of ceph-mon daemons"]
        passfail = 'WARN'
        result.add_check_result(section, check, passfail, summary, detail, recommend)
    else:
        summary = f"Odd number of Monitors"
        detail = [f"Cluster has an odd number of ceph-mon daemons ({num_mons}). This is ideal for the underlying Monitor PAXOS quorum algorithm. Quorum will be maintained as long as {ceil(num_mons/2+1)} ceph-mon daemons are up and healthy."]
        recommend = []
        passfail = 'PASS'
        result.add_check_result(section, check, passfail, summary, detail, recommend)
...
@add_check
def check_report_osdmap(result, data):
    report = data.ceph_report
    section = 'OSD Health'
    ...
    if 'flags_set' in osdmap:
        ...
        detail = [f'Ceph report does not include the osdmap flags_set field.']
    ...
    if r_o_r != running_release:
        ...
        detail = [f"require_osd_release {r_o_r} does not match running release {running_release}"]
        recommend = [f'Unless you are in the middle of a Ceph upgrade, it is strongly recommended to set the required osd release ASAP, e.g. ceph osd require-osd-release {running_release}']
        result.add_check_result(section, check, 'FAIL', summary, detail, recommend)
        result.force_fail = True
    else:
        summary = 'require_osd_release is correct'
        detail = [f"require_osd_release {r_o_r} matches running release {running_release}"]
        result.add_check_result(section, check, 'PASS', summary, detail, recommend)
...
@add_check
def check_report_osd_info(result, data):
    report = data.ceph_report
    section = 'OSD Health'
    check = 'Info'
...
@add_check
def check_report_osd_primary_affinity(result, data):
    report = data.ceph_report
    section = 'OSD Health'
    ...
    if bad:
        ...
        bad = ', '.join(bad)
        detail = [f"OSDs {bad} have primary-affinity != 1. This may be leftover from a device replacement procedure."]
        recommend = [f"Reset the osd primary-affinity to 1 for the listed OSDs."]
    else:
        passfail = 'PASS'
        summary = 'All OSDs have optimal primary-affinity'
        bad = ', '.join(bad)
        detail = [f"All OSDs have the recommended primary-affinity (1). This ensures uniform IO handling across the cluster."]
        recommend = []
...
@add_check
def check_report_osd_weight(result, data):
    report = data.ceph_report
    section = 'OSD Health'
    ...
    if bad:
        ...
        bad = ', '.join(bad)
        detail = [f"OSDs {bad} have weight != 1.0. This may be leftover from a device replacement procedure or attempted data balancing."]
        recommend = [f"Reset the osd weights to 1 for the listed OSDs and enable the upmap balancer if required."]
    else:
        passfail = 'PASS'
        summary = 'All OSDs have optimal weight'
        bad = ', '.join(bad)
        detail = [f"All OSDs have the recommended weight (1). When used in tandem with the upmap balancer, this ensures an optimal data placement."]
        recommend = []
...
@add_check
def check_report_pool_info(result, data):
    report = data.ceph_report
    section = 'Pools'
    check = 'Info'
    ...
    for p in pools:
        ...
        detail.append(f"Pool {p['pool_name']} with {ptype} size {p['size']}, created {create_time}")
    result.add_info_result(section, check, summary, detail)


@add_check
def check_report_capacity_info(result, data):
    report = data.ceph_report
    section = 'Capacity'
    check = 'Info'
    summary = f'Cluster Capacity Info'
    detail = []
    ...
    detail.append(f'OSD total capacity {total_osd_size}. Used: {total_osd_used}. Available: {total_osd_avail}.')

    result.add_info_result(section, check, summary, detail)


@add_check
def check_report_capacity_overfull(result, data):
    report = data.ceph_report
    section = 'Capacity'
    ...
    if percent_used > 0.9:
        passfail = 'FAIL'
        detail.append(f'Cluster is {percent_used:.1%} full, over 90%, risking data loss in case of a node failure.')
        recommend.append(f'Add capacity or delete unneeded data to free space ASAP.')
    elif percent_used > 0.8:
        passfail = 'WARN'
        detail.append(f'Cluster is {percent_used:.1%} full, over 80%. Add capacity soon.')
        recommend.append(f'Add capacity or delete unneeded data to free space.')
    else:
        passfail = 'PASS'
        detail.append(f'Cluster is {percent_used:.1%} full, under 80%, safe for production.')

    result.add_check_result(section, check, passfail, summary, detail, recommend)


@add_check
def check_report_pool_flags(result, data):
    report = data.ceph_report
    section = 'Pools'
    ...
    for f in flags:
        ...
        if pools_missing_flag:
            detail.append(f"Pools missing recommended '{f}' flag: {', '.join(pools_missing_flag)}")

    if passfail == 'FAIL':
        summary = 'Some pools have missing flags'
        recommend = [r"It is strongly recommended to set the nodelete and nosizechange flags to prevent unintended pool changes in production. E.g. ceph osd pool set \<poolname\> nodelete 1"]
        result.add_check_result(section, check, passfail, summary, detail, recommend)
    else:
...
@add_check
def check_report_pool_size(result, data):
    report = data.ceph_report
    section = 'Pools'
    check = 'Pool Sizing'
    ...
    for p in pools:
        ...
        if p['type'] == 1:
            if size < 3:
                ...
                recommend.append(f"Increase size on pool {p['pool_name']} to 3 to minimize the likelihood of data loss.")
            if min_size < 2:
                passfail = 'FAIL'
                detail.append(f"Replicated pool {p['pool_name']} has min_size {min_size}.")
                recommend.append(f"Increase min_size on pool {p['pool_name']} to 2 to minimize the likelihood of data loss.")
        elif p['type'] == 3:
            ...
            if min_size < expected_min_size:
                detail.append(f"Erasure {profile['k']}+{profile['m']} pool {p['pool_name']} has min_size {min_size}.")
                recommend.append(f"Increase min_size on pool {p['pool_name']} to {expected_min_size} to minimize likelihood of data loss.")

    if not len(detail):
        passfail = 'PASS'
        summary = 'All pools have correct min_size setting'
        detail.append(f"All pools have correct min_size setting")
    else:
        passfail = 'FAIL'
...
@add_check
def check_report_pool_autoscale(result, data):
    report = data.ceph_report
    section = 'Pools'
    check = 'Pool Autoscale Mode'

    pools = report['osdmap']['pools']
    ver = report['version'].split('-')[0]
    detail = []
    recommend = []
    ...
    if passfail == 'PASS':
        detail.append(f"All pools have pg_autoscaler disabled. This is the recommended setting for most environments.")
    elif passfail == 'WARN':
        detail.append(f"The following pools have pg_autoscale_mode = on. This may cause unexpected background load during production hours, and is not recommended in most environments. Pools: {', '.join(affected_pools)}")
        recommend.append(f"Set pg_autoscale_mode to 'warn' or 'off' for all pools and set pg_num accordingly.")
...
def check_report_pool_size(result, data):
    ...
    detail = [f'{len(rbd_pools)} RBD pools found. '
...
@add_check
def check_report_pool_min_pgnum(result, data):
    report = data.ceph_report
    section = 'Pools'
    ...
    for p in pools:
        application = len(p['application_metadata']) and \
            list(p['application_metadata'].keys())[0] or None
    ...
    if passfail == 'PASS':
        detail.append(f"All pools have at least one replica or shard per OSD.")

    result.add_check_result(section, check, passfail, summary, detail, recommend)


@add_check
def check_report_pool_crush_domain_buckets(result, data):
    report = data.ceph_report
    section = 'Pools'
    ...
    if passfail == 'PASS':
        ...
        detail.append(f"All pools have CRUSH domain satisfied.")
    else:
...
@add_check
def check_report_pool_zero_weight_buckets(result, data):
    report = data.ceph_report
    section = 'Pools'
    ...
    if passfail == 'PASS':
        ...
        detail.append(f"All pools use CRUSH tree with no zero weight buckets.")
    else:
        how_many = 'All' if len(pools) == failed_count else 'Some'
...
def check_report_pool_crush_domain_buckets(result, data):
    ...
    if passfail == 'PASS':
        ...
        detail.append(f"All pools have balanced CRUSH tree.")
    else:
        how_many = 'all' if len(pools) == failed_count else 'some'
...
@add_check
def check_report_pool_avg_object_size(result, data):
    report = data.ceph_report
    section = 'Pools'
    ...
    for p in pools:
        ...
        if p['type'] == 1:
            ...
            if avg_obj_size < p['stripe_width']:
                profile_name = p['erasure_code_profile']
                passfail = 'WARN'
                detail.append(
    ...
    if passfail == 'PASS':
        ...
        detail.append("For all pools the average object size is higher "
                      "than allocation unit")
    else:
        summary = 'Average object size in some pools is too small'

    result.add_check_result(section, check, passfail, summary, detail,
                            recommend)


@add_check
def check_report_pool_space_amplification(result, data):
    report = data.ceph_report
    section = 'Pools'
...
@add_check
def check_report_pool_cache_tiering(result, data):
    report = data.ceph_report
    section = 'Pools'
    check = 'Cache Tiering'
    ...
    for p in pools:
        ...
        elif p['read_tier'] > 0 and p['write_tier'] > 0 and \
             p['read_tier'] == p['write_tier']:
    ...
    if not len(detail):
        ...
        detail.append(f"No pools have cache tiering enabled.")
        recommend.append(f"Do not enable cache tiering.")
    else:
        passfail = 'WARN'
        summary = 'One or more pools have cache tiering enabled'
        recommend.append(f"Consider disabling cache tiering.")
...
@add_check
def check_report_pg_upmap(result, data):
    report = data.ceph_report
    section = 'OSD Health'
    ...
    if report['osdmap']['pg_upmap']:
        ...
        detail = [f"osdmap contains {len(report['osdmap']['pg_upmap'])} pg_upmap entries, which is not recommended"]
        recommend = [f"pg_upmap is not useful outside of very rare advanced scenarios. It is likely this was used by mistake and confused with pg-upmap-items entries."]
    else:
        passfail = 'PASS'
        summary = 'osdmap pg_upmap list is empty'
        detail = ['pg_upmap is not useful in normal situations, and this cluster is correctly not using it.']
...
@add_check
def check_report_journal_rotational(result, data):
    ...
    if passfail == 'FAIL':
        ...
        detail.append(f"The following OSDs have a rotational db/wal or journal: {', '.join(rotational)}")
        recommend.append(f"Migrate db/wal or journal for affected OSDs to non-rotational (flash) devices.")
    else:
        summary = 'All OSDs have bluefs db/wal or journal on non-rotational device'
        detail.append(f"Bluefs db/wal on non-rotational device is recommended for expected performance.")

    result.add_check_result(section, check, passfail, summary, detail, recommend)


@add_check
def check_report_bluefs_db_size(result, data):
    ...
    for osd in osd_metadata:
        bluefs_db_size = int(osd.get('bluefs_db_size',
                                     min_bluefs_db_size + 1))
        if int(osd.get('bluefs_dedicated_db', 0)) and \
           bluefs_db_size < min_bluefs_db_size:
            passfail = 'FAIL'
            detail.append(f"osd.{osd['id']} bluefs db size {bluefs_db_size}")
            recommend.append(f"Migrate osd.{osd['id']} bluefs db to partition of at least {min_bluefs_db_size} size")
            failed_count += 1
    ...
    if passfail == 'FAIL':
        ...
        detail.append(f"Bluefs db of at least {min_bluefs_db_size} size is recommended for expected performance.")
...
def check_report_bluefs_db_size(result, data):
    ...
    for osd in osd_metadata:
        bluefs_wal_size = int(osd.get('bluefs_wal_size',
                                      min_bluefs_wal_size + 1))
        if int(osd.get('bluefs_dedicated_wal', 0)) and \
           bluefs_wal_size < min_bluefs_wal_size:
            passfail = 'FAIL'
            detail.append(f"osd.{osd['id']} bluefs wal size {bluefs_wal_size}")
            recommend.append(f"Migrate osd.{osd['id']} bluefs wal to partition of at least {min_bluefs_wal_size} size")
            failed_count += 1
    ...
    if passfail == 'FAIL':
        ...
        detail.append(f"Bluefs wal of at least {min_bluefs_wal_size} size is recommended for expected performance.")

    result.add_check_result(section, check, passfail, summary, detail, recommend)


@add_check
def check_report_bluestore_min_alloc_size(result, data):
    osd_metadata = data.ceph_report['osd_metadata']
    section = 'OSD Health'
    ...
    if failed_osds:
        ...
        recommend.append(
            f"Redeploy the reported OSDs with bluestore min_alloc_size 4096"
        )
    else:
...
@add_check
def check_report_host_memory(result, data):
...
@add_check
def check_report_host_swap(result, data):
    osd_metadata = data.ceph_report['osd_metadata']
    section = 'OSD Health'
    check = 'OSD host swap'
    passfail = 'PASS'
    ...
    if passfail == 'PASS':
        ...
        detail.append(
            f"Disabling swap is recommended for expected performance."
        )
    else:
        detail.append(
        ...
        recommend.append(
            f"Disable swap on hosts and increase memory if needed"
        )
...
@add_check
def check_report_num_osdmaps(result, data):
    report = data.ceph_report
    section = 'OSD Health'
    ...
    if num_committed > 1500:
        ...
        detail = [f"Ceph is storing {num_committed} osdmaps, which may indicate that they are not being trimmed correctly. This may be transient during extended periods of backfilling."]
        recommend = [f"If this does not resolve itself when backfilling completes, it can indicate a deeper problem in trimming osdmaps. Consult Clyso support in that case."]
    else:
        passfail = 'PASS'
        summary = 'Cluster osdmaps are trimming correctly'
        detail = [f"Ceph is storing {num_committed} osdmaps, which is within the normal range."]
...
@add_check
def check_report_crush_tunables_optimal(result, data):
    report = data.ceph_report
    section = 'OSD Health'
    ...
    for tunable in optimal.keys():
        ...
        if recommended is not None:
            ...
            detail.append(f'CRUSH tunable {tunable} is currently {tunables[tunable]}, not the recommended {recommended}.')
            recommend.append(f'Set the tunable {tunable} to {recommended}. Note that this may result in significant data movement. Contact support if you are unsure.')
...
@add_check
def check_report_fsmap_info(result, data):
    report = data.ceph_report
    section = 'CephFS'
    check = 'CephFS Info'
    detail = []
    ...
    for fs in filesystems:
        mdsmap = fs['mdsmap']
        detail.append(f"{mdsmap['fs_name']}: data pools {mdsmap['data_pools']}, meta pool {mdsmap['metadata_pool']}, max_mds {mdsmap['max_mds']}")
    result.add_info_result(section, check, summary, detail)


@add_check
def check_report_fsmap_multi_mds(result, data):
    report = data.ceph_report
    section = 'CephFS'
    ...
    for fs in filesystems:
        ...
        if mdsmap['max_mds'] > 1:
            multimds = True
            passfail = 'WARN'
            detail.append(f"Filesystem {mdsmap['fs_name']} has multiple active MDSs (max_mds is {mdsmap['max_mds']}). Multi-MDS should be used with caution and following careful guidance.")
        if mdsmap['max_mds'] > 8:
            passfail = 'FAIL'

    if multimds:
        summary = f"Found CephFS filesystems with multiple active MDS daemons"
        recommend.append("Multiple active MDS daemons should only be enabled if technically justified and following careful guidance.")
    elif has_cephfs:
        summary = f"All CephFS filesystems use a single active MDS"
    else:
...
@add_check
def check_report_osd_cluster_network(result, data):
    report = data.ceph_report
    section = 'OSD Health'
    ...
    if public_ip == cluster_ip:
        ...
        detail = [f"OSDs are using the same IP address for the public and cluster networks. This may have performance implications for busy clusters notably during periods of recovery or backfilling."]
        recommend = [f"Consider adding a dedicated cluster network for internal OSD traffic."]
    else:
        passfail = 'PASS'
        summary = 'Public and Cluster Networks are Separated'
        detail = [f"OSDs are using distinct IP addresses for the public and clusters networks. This is ideal for optimal performance."]
        recommend = []
...
@add_check
def check_report_operating_system(result, data):
    report = data.ceph_report
    ...
    if is_container_deployment:
        ...
        detail = ["Ceph is deployed using containers (cephadm). OS check skipped since container deployments use standardized images."]
        passfail = 'PASS'
        recommend = []
        result.add_check_result(section, check, passfail, summary, detail, recommend)
        return
    ...
    for d in distro_descriptions:
        if d not in osdb['operating_systems']:
            ...
            detail.append(f'{d} is unknown - this may or may not be a stable platform for Ceph.')
            recommend.append(f"Upgrade {d} to a recommended recent OS, including RHEL 9+ or clones, Ubuntu 22.04+, or Debian 12+. See https://docs.ceph.com/en/latest/start/os-recommendations/ for further information.")
            continue
        ...
        if os['status'] != 'Supported':
            ...
            recommend.append(f"{os['recommend']} See https://docs.ceph.com/en/latest/start/os-recommendations/ for further information.")
        else:
