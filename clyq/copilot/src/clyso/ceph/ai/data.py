...
class CephData:
    ...
    def add_ceph_report(self, ceph_report: dict):
        self.ceph_report = ceph_report

    # output from `ceph config dump -f json`
    def add_ceph_config_dump(self, ceph_config_dump: dict):
        self.ceph_config_dump = dict(ceph_config_dump)
    
    # output from `ceph osd tree -f json`
    def add_ceph_osd_tree(self, ceph_osd_tree: dict):
        self.ceph_osd_tree = ceph_osd_tree
    
    def add_ceph_pg_dump(self, ceph_pg_dump: dict):
