...
from clyso.ceph.ai.common import jsoncmd, json_load, CopilotParser
...
def main():
    ...
    parser = CopilotParser(prog='ceph-copilot', description="Ceph Copilot: Your Expert Ceph Assistant.")
    subparsers = parser.add_subparsers(title='subcommands',
                                       description='valid subcommands',
                                       dest='{cluster, pool, toolkit}')
    parser.add_argument('--version',"-v","-V", action='version', version=f'Clyso Ceph Copilot v{__version__}')
    ...
    parser_cluster = subparsers.add_parser('cluster', help='List of commands related to the cluster')
    cluster_subparsers = parser_cluster.add_subparsers(dest="{checkup}",description='valid subcommands', help='additional help')
    cluster_subparsers.required = True;

    # create the parser for the "pool" command
    parser_pool = subparsers.add_parser('pools', help='Operations and management of Ceph pools')
    pool_subparsers = parser_pool.add_subparsers(dest="{pg}", description='valid subcommands for pools', help='additional help')
    pool_subparsers.required = True;

    # create the parser for the "checkup" command
    parser_checkup = cluster_subparsers.add_parser('checkup', help='Perform an overall health and safety check on the cluster')
    ...
    toolkit_subparsers = parser_toolkit.add_subparsers(dest='{list, run}', help='tool help')
    toolkit_subparsers.required = True;

    # Create the parser for the "toolkit list" command
    parser_toolkit_list = toolkit_subparsers.add_parser('list', help='List the included Ceph tools')
    parser_toolkit_list.set_defaults(func=toolkit_list)

    # Create the parser for the "toolkit run" command
    parser_toolkit_run = toolkit_subparsers.add_parser('run', help='Run an included Ceph tool', 
    ...
    parser_toolkit_run.add_argument('args', nargs=argparse.REMAINDER, help='Name of the tool to run and its arguments')
